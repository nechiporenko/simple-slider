(function () {
    ...
    function initSlider(time) {
        ...
        method.requestInterval = function () {//для оптимизации ресурсов браузера будем использовать requestAnimationFrame вместо setInterval
            var requestAnimFrame = (function () {
                return window.requestAnimationFrame || function (callback, element) {
                    handle.timer = setTimeout(callback, 1000 / 60); //fallback для ie9
                };
            })(),
            ...
            function loop() {
                ...
                var is_now = new Date().getTime(),
                delta = is_now - start;
                if (delta >= handle.delay) {
                    method.nextSlide();
                    start = new Date().getTime();
                }
            }
